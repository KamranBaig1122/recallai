...
class MeetingTranscription(models.Model):
    ...
    # Contextual nudges and meeting intelligence (generated by Groq)
    contextual_nudges = JSONField(default=list, null=True, blank=True)  # Array of contextual nudges
    impact_score = models.FloatField(null=True, blank=True)  # Legacy; no longer populated for new meetings
    key_outcomes_signals = JSONField(default=list, null=True, blank=True)  # Short outcome/signal bullets (replaces impact score UX)
    meeting_gaps = JSONField(default=list, null=True, blank=True)  # Gaps / risks identified in the meeting
    open_questions = JSONField(default=list, null=True, blank=True)  # Unresolved or still-unclear items
